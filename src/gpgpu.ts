...
export function computeVelocities(
   ...
   const alignmentThresh =
      (separationDistance + alignmentDistance) / zoneRadius;
   const zoneRadiusSquared = zoneRadius * zoneRadius;

   for (let i = 0; i < input.length; i += 3) {
      const selfPosition: [number, number, number] = [
         positions[i],
         positions[i + 1],
         positions[i + 2],
      ];
      const selfVelocity: [number, number, number] = [
         input[i],
         input[i + 1],
         input[i + 2],
      ];
         ...
         for (let x = 0; x < width; x++) {
            ...
            if (percent < separationThresh) {
               ...
               const adjustedPercent =
                  (percent - separationThresh) / threshDelta;
               ...
               const f =
                  (0.5 - Math.cos(adjustedPercent * PI_2) * 0.5 + 0.5) *
                  deltaTime;
      ...
      if (distToBorderX < borderDistance) {
         forceX =
            ((borderDistance - distToBorderX) / borderDistance) *
            -Math.sign(selfPosition[0]);
      }
      if (distToBorderY < borderDistance) {
         forceY =
            ((borderDistance - distToBorderY) / borderDistance) *
            -Math.sign(selfPosition[1]);
      }
      if (distToBorderZ < borderDistance) {
         forceZ =
            ((borderDistance - distToBorderZ) / borderDistance) *
            -Math.sign(selfPosition[2]);
      }

      velocity = add(
         velocity,
         scale([forceX, forceY, forceZ], borderForce * deltaTime)
      );
      ...
      const rotationAngle =
         0.02 * Math.sin(time / 100 + (i / input.length) * 2 * Math.PI);
      ...
      const rotatedVelocity = [
         rotationMatrix[0][0] * velocity[0] +
            rotationMatrix[0][1] * velocity[1] +
            rotationMatrix[0][2] * velocity[2],
         rotationMatrix[1][0] * velocity[0] +
            rotationMatrix[1][1] * velocity[1] +
            rotationMatrix[1][2] * velocity[2],
         rotationMatrix[2][0] * velocity[0] +
            rotationMatrix[2][1] * velocity[1] +
            rotationMatrix[2][2] * velocity[2],
      ];
