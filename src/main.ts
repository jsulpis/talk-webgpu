...
function loadModel() {
   ...
   loader.load(modelUrl, function (gltf) {
      ...
      for (let i = 0; i < FISHES; i++) {
         ...
         getMaterial(clonedModel).color = new THREE.Color(
            0,
            0.6 + Math.random() * 0.4,
            0.6 + Math.random() * 0.4
         );
...
function animate() {
   ...
   computeElement.innerHTML = compute.duration.toFixed(2) + "ms";
   ...
   renderElement.innerHTML = render.duration.toFixed(2) + "ms";
}
