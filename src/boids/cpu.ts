...
export function useBoidsCPU(initialPositions: Float32Array, initialVelocities: Float32Array, FISHES: number) {
  ...
  function swapPositionsData() {
    const temp = positionsData.read;
    positionsData.read = positionsData.write;
    positionsData.write = temp;
  }

  function swapVelocityData() {
    const temp = velocityData.read;
    velocityData.read = velocityData.write;
    velocityData.write = temp;
  }
  ...
  function compute({
    ...
    computePositions(positionsData.read, positionsData.write, velocityData.read, deltaTime);
    swapPositionsData();

    return {
      positions: positionsData.read,
    };
  }
